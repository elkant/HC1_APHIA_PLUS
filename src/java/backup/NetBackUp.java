 ...
 */
public class NetBackUp extends HttpServlet {

    
    
    //============================This code will get data from the master hc table==================
    
    
    
    String dbname, dbuser, dbpassword,localhost,localhostsplit[];
...
String  filname="";
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            //this code should be backing up the master database and sending it via email..
   ...
   conn.st.executeUpdate(createdeletedtable); 
    ...
    //        GET CURRENT DATE:
            ...
            if (session.getAttribute("username") != null ) {
                ...
                filname=session.getAttribute("username")+"_"; 
                ...
                for (int i = 0; i < myalphabet.length; i++) {
                    try {
                        ...
                        if (f3.exists() && f3.isDirectory()) {
                            path = current_drive + ":\\APHIAPLUS\\HC_SYSTEM\\DATA\\BACKUP2";
    ...
    //executeCmd = "C:\\wamp3\\bin\\mysql\\mysql5.6.12\\bin\\mysqldump --no-create-info --skip-add-drop-table --host=localhost --user="+dbuser+" --password="+dbpassword+" "+dbname+" audit enrollment childage clientmember clientoccupation clientoparea dummy medical_form riskassessmentdetails riskassessmentmain riskreductionmain riskreductiondetails user taskauditor --where=timestamp>='"+startdate+"' -r "+dbpath+"";
                        ...
                        Logger.getLogger(NetBackUp.class.getName()).log(Level.SEVERE, null, ex);
                    }
                }
    ...
    //                SEND TO THE MAIL BACKED UP DATA.
                        ...
                        if (isInternetReachable() == true) {
                            ...
                            if (dt.Sendattachment(full_dates, dbpath, computername, senderofmail, filname) == true) {
                                //do an insert
    ...
    //        response.setHeader("Content-Disposition", "attachment; filename=Countybackup_"+date+".sql");
    ...
    //        RequestDispatcher dispatcher = getServletContext().getRequestDispatcher("/backupdata.jsp");
