 ...
 * @author SIXTYFOURBIT
 */
public class hc_loadAttendance extends HttpServlet {
...
//=====================================================================
     ...
     if (session.getAttribute("userid") == null) {
        response.sendRedirect("index.jsp");
            ...
            System.out.println(" GROUP ID::"+session.getAttribute("s_group_name"));
            System.out.println(" MARKED DATE::"+session.getAttribute("hc_ssh_marked_date"));
            System.out.println(" MARKED DATE::"+session.getAttribute("hc_ssh_marked_date"));
            //the final query to be adapted
            // String to check if the name entered is the first name
             String studchecker="select * from register_attendance join member_details on register_attendance.member_id=member_details.member_id where member_details.group_id='"+session.getAttribute("s_group_name")+"' AND  register_attendance.marked_date='"+session.getAttribute("hc_ssh_marked_date").toString().replace(" ","-") +"'"
                    + " AND  ( member_details.sur_name='"+student+"' OR member_details.first_name='"+student+"' OR member_details.mname='"+student+"')AND member_details.year='"+session.getAttribute("year") +"' && member_details.period='"+session.getAttribute("period") +"' && member_details.month='"+selected_month +"'";
         
           String loadavailability="select * from register_attendance join member_details on register_attendance.member_id=member_details.member_id where register_attendance.marked_date='"+session.getAttribute("hc_ssh_marked_date").toString().replace(" ","-") +"'AND member_details.year='"+session.getAttribute("year") +"' && member_details.period='"+session.getAttribute("period") +"' && member_details.month='"+selected_month +"'";
           ...
           System.out.println(loadavailability);
...
//________________________________________________________________________________________________________
           ...
           //session.getAttribute("s_group_name") gives the group id and not group name...Ignore the naming
           
           
           String session_content="Select * from session where group_id='"+session.getAttribute("s_group_name")+"' AND marked_date='"+session.getAttribute("hc_ssh_marked_date").toString().replace(" ", "-") +"' ";
           
           System.out.println(session_content);
...
//                ---------------START AND END DATE------------------------------------------------
                                         String selector_dater="select * from new_topic where facilitator_id='"+conn.rs.getString("facilitator_id")+"'";  
...
String end_date=dy2+"- "+mn2+"- "+yr2;
  ...
  session.setAttribute("respo", "<font color=\"red\">No student was found under that Name</font>");
           ...
           // session.setAttribute("reg_topic","Self Awareness");
            ...
            Logger.getLogger(hc_loadAttendance.class.getName()).log(Level.SEVERE, null, ex);
        }
