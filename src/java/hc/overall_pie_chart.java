 ...
 */
public class overall_pie_chart extends HttpServlet {
HttpSession session;
 String county_name="";
...
conn.rs0=conn.st0.executeQuery(county_selector);
while(conn.rs0.next()){
county_name=conn.rs0.getString("county_name");
//^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
//^^^^^^^^^^^^^^^^^^^^^GET ALL THE DETAILS AND RELATIONS HERE^^^^^^^^^^^^^^^^^^^^^^
String district_selector2="select * from district where county_id='"+conn.rs0.getString("county_id")+"'";
...
JFreeChart chart = ChartFactory.createPieChart("Overall Completion Rate Pie Chart.("+period_name+")"+yea+"                Total Indivinduals: "+total,county_chart,true,true,false);
...
//System.out.println("completed :"+all_per+"%");
//System.out.println("partial :"+partial_per+"%");
//System.out.println("incomplete :"+incomplete_par+"%");
//System.out.println("alleer:"+aller+"%");
//System.out.println("alleer2:"+aller_partial+"%");
//System.out.println("alleer:"+aller_incomplete+"%");
