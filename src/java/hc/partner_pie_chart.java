 ...
 */
public class partner_pie_chart extends HttpServlet {
HttpSession session;
 String partner_name="",district_id="";
...
//    ^^^^^^^^^^^^^^ GET PARTNER ID^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
 ...
 conn.rs2=conn.st2.executeQuery(partner_selector);
 if(conn.rs2.next()==true){
     partner_name=conn.rs2.getString("partner_name");
 }
...
//     ^^^^^^^^^^^^^^^^ GET GROUP DETAILS I.E GROUP ID AND TARGET POPULATION ID
 String group_selector="select * from groups where district_id='"+district_id+"' && partner_id='"+partner_id+"'";
...
if(all==0){
//    session.setAttribute("partner_select","<font color=\"red\">"+partner_name+" Partner has no registered member whose attendance has been marked.</font>");
//    response.sendRedirect("partner_pie_chart.jsp");
}
else{
...
JFreeChart chart = ChartFactory.createPieChart(partner_name+" Completion Rate Pie Chart in  "+district_name+" ("+period_name+")"+yea+"               Total Peers: "+total,county_chart,true,true,false);
